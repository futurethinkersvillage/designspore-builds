...
function ringAround(verts, nvec, k) {
  ...
  // The k members are mathematically equidistant from nvec, so which one wins
  // the `d` sort above (and therefore sets the angular reference e1) is decided
  // by last-ULP float noise — it differs between JS and Python. Rotate the ring
  // so the lowest vertex index leads: same cyclic order and direction, but a
  // deterministic starting offset. Keeps the Python port at exact parity.
  const s0 = ring.indexOf(Math.min(...ring));
  return ring.slice(s0).concat(ring.slice(0, s0));
}
