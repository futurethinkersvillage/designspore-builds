...
const DEFAULT_SCENES: TourScene[] = [
  ...
  {
    id: "sauna",       title: "Sauna",        type: "sphere", image: `${BASE}/Sauna-1_PANO_0001-Sauna-1_PANO_0035_blended_fused.jpg`,
  ...
  {
    id: "cabin-3",     title: "Cabin 3",      type: "sphere", image: `${BASE}/Cabin-3.2_PANO_0001-Cabin-3-2-1.jpg`,
  ...
  {
    id: "shower-house", title: "Shower House", type: "sphere", image: `${BASE}/Shower-House-1_PANO_0001-Shower-House-1_PANO_0035_blended_fused-2-1.jpg`,
    initialYaw: 319.11, initialPitch: -15.69,
    links: [{ nodeId: "top-view", yaw: 286.829, pitch: 26.504, title: "Top View" }],
  },
  {
    id: "Gazebo",      title: "Gazebo",        type: "sphere", image: `${BASE}/Gazebo-5_PANO_0001-Gazebo-5_PANO_0035_blended_fused-2-1.jpg`,
    initialYaw: 222.49, initialPitch: -30.83,
    links: [{ nodeId: "top-view", yaw: 246.296, pitch: -18.953, title: "Top View" }],
  },
  {
    id: "trophy-mountain", title: "Trophy Mountain", type: "sphere", image: `${BASE}/TrophyMountain_blended_fused-2-1.jpg`,
    initialYaw: 0, initialPitch: 0,
    links: [{ nodeId: "top-view", yaw: 303.520, pitch: -9.052, title: "Wells Gray Resort" }],
  },
  {
    id: "Winter",      title: "Winter",        type: "sphere", image: `${BASE}/Winter15-2-1.jpg`,
    initialYaw: 259.13, initialPitch: -81.42,
    links: [{ nodeId: "top-view", yaw: 156.515, pitch: -63.088, title: "Summer View" }],
  },
  {
    id: "Dome-Interior", title: "Dome Interior", type: "flat",   image: `${BASE}/Dome-Interior-1.jpg`,
    links: [{ nodeId: "top-view", yaw: 0, pitch: 0, title: "Top View" }],
  },
  {
    id: "Lake-and-Field", title: "Lake and Field", type: "sphere", image: `${BASE}/DomeLake1_blended_fused-2-1.jpg`,
  ...
  {
    id: "Campsite",    title: "Campsite",      type: "flat",   image: `${BASE}/Picnic-Tables-at-the-Creek.jpg`,
    links: [{ nodeId: "top-view", yaw: 0, pitch: 0, title: "Top View" }],
  },
];

type Calibrations = Record<string, { yaw: number; pitch: number }>;

function loadSaved(): Calibrations {
  try {
    return JSON.parse(localStorage.getItem("tour-calibrations") ?? "{}");
  } catch {
    return {};
  }
}
...
export default function TourAdminPage() {
  ...
  const [livePos, setLivePos] = useState<{ yaw: number; pitch: number } | null>(null);
  const [calibrations, setCalibrations] = useState<Calibrations>({});
  const [copied, setCopied] = useState(false);
  const initialized = useRef(false);

  // Load localStorage once on first render (avoids SSR mismatch)
  const initRef = useCallback((node: HTMLDivElement | null) => {
    if (node && !initialized.current) {
      initialized.current = true;
      setCalibrations(loadSaved());
    }
  }, []);
  ...
  function captureView() {
    if (!livePos) return;
    const next = { ...calibrations, [activeSceneId]: { yaw: livePos.yaw, pitch: livePos.pitch } };
    setCalibrations(next);
    localStorage.setItem("tour-calibrations", JSON.stringify(next));
  }
  ...
  function copyConfig() {
    const scenes = DEFAULT_SCENES.map((scene) => {
      ...
      return { ...scene, initialYaw: cal.yaw, initialPitch: cal.pitch };
    });

    const lines: string[] = ["const SCENES: TourScene[] = ["];
    for (const s of scenes) {
      const yawPitch = s.type === "sphere" && s.initialYaw !== undefined
        ? `\n    initialYaw: ${s.initialYaw}, initialPitch: ${s.initialPitch},`
        : "";
      lines.push(`  {
    id: "${s.id}",
    title: "${s.title}",
    type: "${s.type}",
    image: \`\${BASE}/${s.image.split("/").pop()}\`,${yawPitch}
    links: ${JSON.stringify(s.links, null, 6).replace(/\n/g, "\n    ")},
  },`);
    }
    lines.push("];");
  ...
  // Build scenes with calibrations applied for the live viewer
  const scenes = DEFAULT_SCENES.map((scene) => {
    const cal = calibrations[scene.id];
    if (!cal || scene.type === "flat") return scene;
    return { ...scene, initialYaw: cal.yaw, initialPitch: cal.pitch };
  ...
  return (
    <div className="fixed inset-0 flex bg-warm-dark" ref={initRef}>
      {/* Tour viewer */}
      ...
      <div className="flex w-72 shrink-0 flex-col gap-4 overflow-y-auto border-l border-white/10 bg-black/80 p-4 backdrop-blur-sm">
        ...
        <div className="rounded-lg bg-white/5 p-3 font-mono text-xs">
          ...
          {livePos ? (
            ...
            <p className="text-white/30">{activeScene?.type === "flat" ? "flat photo — no camera" : "drag the panorama to move"}</p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2">
          {activeScene?.type === "sphere" && (
            ...
            >
              Set current view as initial
            </button>
          )}
          {savedForScene && (
            <button
              onClick={clearScene}
              className="rounded-lg border border-white/10 px-4 py-2 text-sm text-white/60 hover:border-white/30 hover:text-white transition-colors"
            >
              Reset to default
            </button>
          )}
        ...
        {/* Copy config */}
        <div>
          <p className="mb-1.5 text-[10px] font-medium uppercase tracking-widest text-white/40">Export</p>
          <p className="mb-3 text-[11px] text-white/40">
            Copies the full SCENES array with your calibrations applied. Paste into{" "}
            <code className="text-white/60">tour/page.tsx</code> to make them permanent.
          </p>
          <button
            onClick={copyConfig}
