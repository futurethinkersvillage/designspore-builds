          ...
          <div className="overflow-x-auto -mx-4 px-4">
            <div className="min-w-[580px]">
                ...
                {["Cabin Max", "Cabin", "RV"].map((h, i) => (
                  <div
                    ...
                    className={`text-center py-3 px-2 rounded-t-xl text-sm font-medium ${
                      i === 0 ? "bg-amber/15 text-amber border border-amber/25 border-b-0" : "bg-white/[0.03] text-white/50 border border-white/[0.07] border-b-0"
                    }`}
                  >
                    {h}
                    {i === 0 && (
                      <span className="block text-[9px] uppercase tracking-[0.15em] text-amber/60 mt-0.5">Most complete</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              {BENEFITS.map((row, i) => (
                <div
                  key={i}
                  className={`grid grid-cols-[2fr_1fr_1fr_1fr] gap-px ${
                    i === BENEFITS.length - 1 ? "" : ""
                  }`}
                >
              ...
              <div className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-px">
                ...
                {["Inquire →", "Inquire →", "Inquire →"].map((label, j) => (
                  <div
                    key={j}
                    className={`py-4 px-2 flex items-center justify-center rounded-b-xl ${
                      j === 0 ? "bg-amber/10 border-x border-b border-amber/25" : "bg-white/[0.02] border-x border-b border-white/[0.07]"
                    }`}
                  >
                    <a
                      href="#inquiry"
                      className={`text-xs font-medium ${j === 0 ? "text-amber hover:text-amber/80" : "text-white/40 hover:text-white/60"} transition-colors`}
                    >
                      {label}
                    </a>
                  </div>
        ...
        <motion.div {...FI} className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              ...
              >
                <span className="inline-block rounded-full bg-amber/10 border border-amber/20 w-10 h-10 flex items-center justify-center text-amber text-xs font-mono mb-5">
                  {s.step}
                </span>
