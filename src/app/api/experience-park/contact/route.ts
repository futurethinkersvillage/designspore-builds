...
const ROLE_LABELS: Record<string, string> = {
  partner: "A potential partner (event, program, or build)",
  investor: "A potential investor",
  guest: "A future guest / visitor",
  ...
  const tableRows = rows
    .map(
      ...
      <tr>
        ...
        <td style="padding:8px 12px;border:1px solid #e5e7eb;font-size:13px;font-family:sans-serif;white-space:pre-wrap">${value}</td>
      </tr>`
    )
    .join("");
  return `
    <div style="font-family:sans-serif;max-width:640px">
      <div style="background:#fff7ed;border:2px solid #ea824e;border-radius:10px;padding:16px 20px;margin-bottom:20px">
        ...
        <div style="font-size:13px;color:#374151;margin-top:4px">Submitted the Get Involved form on the Experience Park deck.</div>
      </div>
...
export async function POST(req: NextRequest) {
  ...
  if (!res.ok) {
    ...
    console.error("experience-park contact: resend error", err);
